...
//******************** MatchOrders function ********************//
    ...
    returns (bool){
        ...
        while (BuyListpeak(_AuctionID).Price >= SellListpeak(_AuctionID).Price){
            ...
            require (temp_BestBid.Price - temp_BestAsk.Price != 0);
...
//*******************  maxheap_delete () ***************************//
    ...
    function maxheap_delete (uint256 _AuctionID) public returns (OrderStruct memory) 
    {
        ...
        if (Auctions[_AuctionID].BuyList.length == 1) {                                      // if the heap has only one items
            ...
            delete Auctions[_AuctionID].BuyList[0];  
            //Auctions[_AuctionID].BuyList.length--;                                        //the only element of the heap is removed and returned "!SYNTAX!"
        ...
        OrderStruct memory result = Auctions[_AuctionID].BuyList[0]; //the element on the of the heap is placed in the variable called hold
        ...
        delete Auctions[_AuctionID].BuyList[Auctions[_AuctionID].BuyList.length -1];
...
//*******************  minheap_delete () ***************************//
    ...
    //then the heap is reordered uising the heapifyDown method
    function minheap_delete (uint256 _AuctionID) public returns (OrderStruct memory)
    //function minheap_delete () public view returns (uint256 _price)
    {
        ...
        if (Auctions[_AuctionID].SellList.length == 1) {                               // if the heap has only one item
            OrderStruct memory result = Auctions[_AuctionID].SellList[0];  //"!SYNTAX!"
            delete Auctions[_AuctionID].SellList[0];                                   //the only element of the heap is removed and returned  "!SYNTAX!"
            //Auctions[_AuctionID].SellList.length--;   
       ...
       // _volume = BuyList[0].Volume;
        ...
        delete Auctions[_AuctionID].SellList[Auctions[_AuctionID].SellList.length -1]; 
        //Auctions[_AuctionID].SellList.length--;                  //"!SYNTAX!"
        minheap_heapifyDown(_AuctionID);                           //now the heapifyDown is called to restore the ordering of the heap "!SYNTAX!"
        return (result);        
    }
