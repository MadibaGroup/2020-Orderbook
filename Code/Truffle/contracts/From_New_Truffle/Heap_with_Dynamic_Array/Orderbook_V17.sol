...
//******************** MatchOrders function ********************//
    ...
    function Rrefund_unexecuted_Buy_orders (uint256 _AuctionID) internal returns (bool) {
        ...
        require (Auctions[_AuctionID].BuyList.length != 0, "BuyList is empty.");
        
        uint i = Auctions[_AuctionID].BuyList.length;
    ...
    function Rrefund_unexecuted_Sell_orders (uint256 _AuctionID) internal returns (bool) {
        ...
        require (Auctions[_AuctionID].SellList.length != 0, "SellList is empty.");
        
        uint i = Auctions[_AuctionID].SellList.length;
        while (Auctions[_AuctionID].SellList.length != 0)
        {
            TokenBalance[Auctions[_AuctionID].SellList[i].Sender][Auctions[_AuctionID].Token] -= Auctions[_AuctionID].SellList[i].Price;
