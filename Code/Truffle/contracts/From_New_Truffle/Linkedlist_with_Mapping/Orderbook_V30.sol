...
//(!!!!!!!! The second version I created !!!!!!!!)
// ATTENTION:::::::: Orderbook29 is the MAIN Linkedlist with MApping

contract Orderbook_V30{
...
//******************** InsertBid() function ********************//
    ...
    function InsertBid(address _sender, uint256 _price, uint256 _volume, uint256 _auxprice) public returns (bool)
    {
        ...
        while (current.id != BuyTail &&  _auxprice < current.AuxPrice)
        {  current = BuyList[current.next]; }  
        
        if (current.id == BuyHead && _auxprice < current.AuxPrice)
        {
        ...
        if (current.id == BuyTail && _auxprice < current.AuxPrice)
        {
            BuyList_InsertAfter(current.id, _sender, _price,  _volume,  _auxprice );
            return true;
        }
...
//*******************  InsertAsk () ***************************//
    ...
    function InsertAsk (address _sender, uint256 _price, uint256 _volume, uint256 _auxprice) public returns (bool)
    {
        ...
        while (current.id != SellTail &&  _auxprice > current.AuxPrice)
        {  current = SellList[current.next]; }  
        
        if (current.id == SellHead && _auxprice > current.AuxPrice)
        {
        ...
        if (current.id == SellTail && _auxprice > current.AuxPrice)
        {
            SellList_InsertAfter(current.id, _sender, _price,  _volume,  _auxprice );
            return true;
        }
