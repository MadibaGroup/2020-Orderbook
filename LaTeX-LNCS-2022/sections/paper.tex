...
\emph{Order-driven} markets are a  group of trade execution systems where offers to buy (\emph{bids}) and sell (\emph{offers}/\emph{asks}) from many traders are placed as orders in an order book and executed based on matching rules enforced by the market. These markets can be \emph{continuous}, with buyers/sellers at any time adding orders to the order book (\emph{makers}) or executing against an existing order (\emph{takers}); or they can be \emph{called}, where all traders submit orders within a window of time and orders are matched in a batch (like an auction). Table~\ref{tab:exchanges} presents three types of order-driven exchanges:
...
\end{itemize}
Despite having a superior threat model, fully on-chain order-driven exchanges are slow, difficult to regulate and implement (see Table~\ref{tab:exchanges}). In this paper, we focus on benchmarking an order book for the public blockchain Ethereum.
