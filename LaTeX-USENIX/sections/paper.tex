...
\textblue{The only other paper similar to ours is TEX~\cite{khalil2019tex}. }
...
\textblue{Mahsa: Partial Filling: we ran the worst case matching test with random volumes between 1 and 10 on linked list with mapping (which handles maximum pairs of 80 orders with single volumes).  With partial filling, it can clear the book that contains 70 pairs at max (which on average ends up being ~118 trades). SO it is moderately more expensive}
