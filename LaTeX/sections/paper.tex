...
A priority queue can be implemented using many of the data structures that differ in performance and costs. In order to choose the appropriate data structure, we implemented and examined five different generic priority queues that support four main functions (see Table~\ref{tab:PQ_API}). We discuss each implementation below. 
...
\item\textbf{{Mapping with Keys Stored in a Heap.}} This priority queue stores items inside a Solidity mapping that maps them to mapping keys (256-bit integers). These keys are then stored in a sorted heap (based on the value they are mapped to) that is implemented with a dynamically-sized array.

%---------+++++++++++++++++++-----------------

\item\textbf{{Linked List.}} Here we used a sorted singly linked list to implement the priority queue. A linked list is a linear data structure made of a chain of items (also known as nodes) where each item contains a value and a pointer to the next node in the list. In an object-oriented programming language a linked list can be easily implemented with a recursive struct; struct that contains a member variable in its own type. However, Solidity does not support recursive structs because the size of the struct has to be finite. So in our implementation, each node is represented by a smart contract that contains the data and a pointer of its own type to its next smart contract in the list.

%---------+++++++++++++++++++-----------------

\item\textbf{{Linked List with Mapping.}} Here we used Solidity mapping to implement a sorted doubly linked list priority queue where every node points to it previous \textbf{and} next node in the list. The Linked List entries are first wrapped in an struct data type that contains (i) the unique id of that element, (ii) the unique id of its previous item in the list, and (iii) the unique id of its next item in the list. These structs are then stored in the mapping indexed by item id. 
...
In the next step, we used Javascript for testing by leveraging the Mocha testing framework and performed performance tests on the five priority queues. We entered 50 pseudo-random integers between 1 and 200 to the priority queues and obtained the associated gas costs for 200 transactions. Figure~\ref{fig:random_insertion} represents the gas costs variations for entering 50 unsigned integers to the five priority queues; the x-axis is the place in line (\eg the10th number entered in the priority queue) and the and y-axis is the cost of that transaction in gas.
...
%Truffle gas limit for the tests: 11741495
...
% Below is the same table but with "Full, Partial,None" for refunds
...
There are two main ways to implement each of the \textit{linked list} and \textit{linked list with mapping} priority queues. In this section, we perform a comprehensive comparison between these two with the purpose of choosing the proper data structure for implementing the call market.
