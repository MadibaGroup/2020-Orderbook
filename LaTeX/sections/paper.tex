...
% +++++++++++++%
% so in our case, let's take the first row of the above table:

%The transaction consumes 2,518,131 gas
%The gas limit is set up at  11,741,495
%Firstly, they sender get back (11,741,495 - 2,518,13) = 9,223,364 units of unused gas.
%However, the amount of gas that has been used in the contract is 2,518,131 and since 9,223,364 >2,518,131/2, the REFUND generated will be 2,518,131/2= 1,259,065.
%
%The total gas cost of transaction estimated by estimateGas is 3,283,480 
%estimateGas - gasUsed = gasRefund => 3,283,480 - 2,518,131 = 765,349
%
%So we can argue that the estimateGas() gives us an approximate total cost of the transaction and it contains the capped refund counter.


% error in estimateGas:  
%   https://ethereum.stackexchange.com/questions/266/what-are-the-limitations-to-estimategas-and-when-would-its-estimate-be-considera
%   https://github.com/trufflesuite/ganache-core/issues/26


%from solidity readdocs: estimateGas Will call estimate the gas a method execution will take when executed in the EVM without. The estimation can differ from the actual gas used when later sending a transaction, as the state of the smart contract can be different at that time.
...
Based on Table~\ref{tab:LLvsLLMapping}, the linked list with \texttt{Self-destruct()} appears to be a good candidate for the call market implementation. However, as it can be seen in Figure~\ref{fig:random_insertion}, the associated cost for inserting elements into this priority queue is significantly greater than the linked list with mapping as insertion is identical to creating new contracts. Accordingly, we choose to implement the call market with the linked list with mapping that uses the  the \texttt{delete()} method to clear up the chain every time the highest priority element is removed from the list. In overall this priority queue introduces a moderate gas cost for both insertion (\ie order submission) and removal (\ie matching the orders). 
