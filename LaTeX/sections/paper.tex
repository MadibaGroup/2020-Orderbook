...
Every a trade happens inside the match function, the best bid and ask order have to be removed from the priority queue. There are two different ways to do so; (i) we can only update the head and tail pointers of the linkedlist and/or (ii) we can remove the mapping elements that contain best bid and ask orders from storage completely as well as updating the pointers of the linkedlist. \par

Pros and Cons:

Cons: Each order structs contains eight variables of multiple types and removing them from the storage is identical to setting eight variables to zero. Despite the gas refunds (\texttt{SSTORE} clear refunds 15.000 gas~\cite{wood2014ethereum}) this would yet increase the gas cost which then lowers down the maximum number of orders the \texttt{Match()} method could handle in this case. However, not removing uanessacry data on the Ethereum blockchain is not the best design practice. For this reason, we clear the best bid and ask orders every time a trade happens.
