...
% = = = = = = = = = = = = = = = = = = = = =  = = =% 
\subsection{Heap with Dynamic Array}

A Heap is a type of binary tree data structure that comes in two forms of a (i) Max-Heap and (ii) Min-Heap. All the nodes of tree are in a specific order and the root of the tree always represents the highest priority item of the data structure (the largest and smallest values in the Max-Heap and Min-Heap respectively). We implemented a priority queue with a Heap that itself is implemented using Solidity dynamically-sized array.  
% = = = = = = = = = = = = = = = = = = = = =  = = =% 

\subsection{Heap with Static Array}

%Here, we implemented a priority queue using the Heap that itself is stored in Solidity static arrays. Every time the priority queue contract i
...
% = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =  %
 ...
 In order to choose the best priority queue ()

Note that we have to carefully choose a proper implementation as they have different performance and costs. In order to examine 
In order to examine the cost and performanmc

%In order to enable our Call Market contract to act as an interface with any priority queue, we implemented generic types of priority queues that support four main functions (see Table~\ref{tab:PQ_API}). Followings describe each implementation. 
