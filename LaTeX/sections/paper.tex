...
% do the followings for the worst case matching: (Done!)
...
% Have one chart with all the worst case submission and one for random case submission (Done!)
...
\item \textbf{Creating a Data-Contract that Stores the Mapping.} Another design pattern is to create a smart contract that only stores the \texttt{UnavailableBalance} mapping every time the market opens. This data contract will be killed at the end of the matching process.
